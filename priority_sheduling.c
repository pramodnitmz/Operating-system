...
int main(int argc, char *argv[]) {
    ...
    while(fscanf(fp, "%d,%d,%d,%d", &proc[pc].pid, &proc[pc].priority, &proc[pc].at, &proc[pc].bt) != EOF) {
    ...
    puts("PID\tPRI\tTAT\tWT");
    printf("%d\t%d\t%d\t%d\n", proc_copy[0].pid, proc_copy[0].priority, proc[0].tat, proc[0].wt);
    for(int i = 1; i <pc; i++) {
        proc[i].tat = proc[i].complete - proc[i].at;
        proc[i].wt = proc[i].tat - proc_copy[i].bt;
        printf("%d\t%d\t%d\t%d\n", proc_copy[i].pid, proc_copy[i].priority, proc[i].tat, proc[i].wt);
        avg_tat += proc[i].tat;
        avg_wt += proc[i].wt;
    }
